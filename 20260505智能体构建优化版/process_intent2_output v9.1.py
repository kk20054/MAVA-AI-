def main(args: dict) -> dict:
    ...
    ─────────────────────────────
    v9.1 优化:
    - 修复 MAX_LEVELS=3 在底层指标场景下的截断风险：显式 ind_name 命中的指标层级必须保留
    - 明细/归因/贡献场景下，对显式父指标按映射表向下补充子层级，资产可覆盖到 L4，损益/负债可覆盖到 L5
    - 数据量控制改为优先裁剪非显式层级，避免银团贷款、活期存款-员工等叶子指标被误删
    - 保留 v9.0 的 query_profile、日期映射和比较属性补全
    v9.0 优化:
    - 增加 query_profile 语义侧写，识别经营概况、异动归因、贡献结构、趋势、风险预警等场景
    - 当 LLM 参数抽取偏空时，按银行经营分析默认口径补齐 Revenue/Assets/Liabilities 与 L1/L2
    - 保留 v8.1 的日期映射、比较属性补全、数据量控制逻辑，确保可直接替换试跑
    ...
    QUERY_FAMILY_RULES = {
        'overview': ['整体', '总体', '经营概况', '经营情况', '综合分析', '全貌', '概览', '业绩表现'],
        'variance': ['异动', '波动', '大幅', '显著', '变化最大', '变动最大', '拉动', '拖累', '归因'],
        'contribution': ['贡献', '占比', '结构', '构成', '主要来源', '来源'],
        'trend': ['趋势', '走势', '近三个月', '连续', '逐月'],
        'alert': ['预警', '风险', '异常', '下滑', '下降', '减少', '流失', '偏弱'],
        'detail': ['明细', '拆解', '下钻', '细分', '子项'],
    }
    ...
    def _infer_query_profile(query_text):
        q=str(query_text or '').lower()
        families=[]
        for fam,words in QUERY_FAMILY_RULES.items():
            if any(w.lower() in q for w in words):
                families.append(fam)
        has_oic=bool(re.search(r'\b\d{8,9}\b',q))
        broad=any(f in families for f in ('overview','variance','alert')) or not q.strip()
        need_detail=any(f in families for f in ('detail','contribution','variance'))
        prefer_trend='trend' in families
        return {
            'families': families,
            'has_oic': has_oic,
            'broad_overview': broad,
            'need_detail': need_detail,
            'prefer_trend': prefer_trend,
        }
    ...
    user_query=str(args.get('query','') or args.get('user_query','') or '')
    ...
    ai_json['_query_profile']=_infer_query_profile(user_query)
    ai_json['_explicit_ind_name']=_norm_list(ai_json.get('ind_name',[]))
    ...
    def fill_overview_fallback(pj,mt):
        ...
        qp=pj.get('_query_profile',{}) if isinstance(pj.get('_query_profile',{}),dict) else {}
        if not types and qp.get('broad_overview'):
            types=[k for k in ('Revenue','Assets','Liabilities') if k in mt]
            if types: pj['type']=types
        if not lvls and qp.get('broad_overview'):
            lvls=["1","2"]; pj['level']=lvls
        if ind: return pj
        if not types:
    ...
    def enrich_params(pj,mt):
        ...
        if not isinstance(types,list): types=[types] if types else []
        qp=pj.get('_query_profile',{}) if isinstance(pj.get('_query_profile',{}),dict) else {}
        explicit_codes=set(_norm_list(pj.get('_explicit_ind_name',[])))
        if not types and qp.get('broad_overview'):
            types=[k for k in ('Revenue','Assets','Liabilities') if k in mt]
            pj['type']=types
        try: li=sorted({int(l) for l in pj.get('level',[])})
        except: li=[]
        if not li and qp.get('broad_overview'):
            li=[1,2]; pj['level']=['1','2']
        if qp.get('need_detail') and li and max(li)<3:
            li=sorted(set(li+[3])); pj['level']=[str(x) for x in li]
        ...
        for tk in types:
            ...
            # 显式指标保真：LLM 已经命中的指标，必须把真实层级带入，避免后续 cap 过滤。
            for c in (explicit_codes|ac):
                if c in c2l:
                    li=sorted(set(li+[c2l[c]]))
            # 明细/归因/贡献类：如果用户命中的是父指标，向下补充可用子层级。
            # 宽泛经营概况不默认拉满底层，避免一次性请求过多单元格。
            if qp.get('need_detail') and ac:
                descendant_levels=set()
                stack=list(ac)
                visited=set()
                while stack:
                    pc=stack.pop()
                    ...
                    for cc in p2c.get(pc,[]):
                        descendant_levels.add(c2l.get(cc,0))
                        stack.append(cc)
                descendant_levels={x for x in descendant_levels if x}
                if descendant_levels:
                    max_depth = 5 if tk in ('Revenue','Liabilities') else 4
                    li=sorted(set(li+[x for x in descendant_levels if x<=max_depth]))
            if li:
                for tl in li:
        ...
        pj['level']=[str(x) for x in sorted(set(li))] if li else pj.get('level',[])
    ...
    def cap_data_volume(pj,mt):
        ...
        explicit_levels={_get_code_level(c,mt) for c in explicit_codes}
        explicit_levels={x for x in explicit_levels if x}
        if explicit_levels:
            li=sorted(set(li)|explicit_levels)
        n_d=len(dates); na=max(len(attrs),1)
        is_trend_query=n_d>=3
        em=TREND_MAX_LEVELS if is_trend_query else (DETAIL_MAX_LEVELS if qp.get('need_detail') or explicit_levels else MAX_LEVELS)
        while len(li)>em:
            removable=[x for x in li if x not in explicit_levels]
            if not removable: break
            li.remove(max(removable))
        if len(li)<MIN_LEVELS and li and not is_trend_query:
            ...
            li=sorted(set(li))
        al=set(li)
        if ind_names: ind_names=[c for c in ind_names if _get_code_level(c,mt) in al or c in explicit_codes]
        ni=len(ind_names) if ind_names else len(types)*len(li)*5
        if ni*n_d*na>MAX_CELLS_EST and len(li)>1:
            removable=[x for x in li if x not in explicit_levels]
            if removable:
                li.remove(max(removable)); al=set(li)
                if ind_names: ind_names=[c for c in ind_names if _get_code_level(c,mt) in al or c in explicit_codes]
        if ind_names and len(ind_names)>MAX_INDICATORS:
            explicit_part=[c for c in ind_names if c in explicit_codes]
            rest=[c for c in ind_names if c not in explicit_codes]
            ind_names=(explicit_part+rest)[:MAX_INDICATORS]
    ...
    # 内部保真字段只用于本脚本层级控制，不透传给接口。
    ai_json.pop('_explicit_ind_name', None)
