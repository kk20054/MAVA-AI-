...
def _unwrap_results(raw):
    ...
    if not isinstance(parsed, dict):
        return []
    # 兼容不同工作流节点命名：联网节点常见 raw_result/result/web_result/final_result
    for key in ("raw_result", "final_result", "web_result", "result", "results", "data", "items"):
        if key in parsed:
...
def main(args: dict) -> dict:
    ...
    raw_result = first_arg("web_result", "raw_result", "result", "final_result", "results")
    ...
    if not results:
        return {
            ...
            "diagnostics": json.dumps({
                ...
                "hint": "请确认联网检索结果已映射到 raw_result/web_result/result 任一字段。",
            }, ensure_ascii=False),
        }
