...
def _unwrap_results(raw):
    ...
    if not isinstance(parsed, dict):
        return []
    for key in ("final_result", "bks_result", "result", "results", "data"):
        if key in parsed:
...
def main(args: dict) -> dict:
    ...
    """
    raw_result = args.get("bks_result", args.get("result", ""))
    ...
    if not results:
        return {"final_result": json.dumps([], ensure_ascii=False), "has_result": False, "diagnostics": "empty_bks_results"}
