def main(args: dict) -> dict:
    ...
    def generate_recommendations():
        ...
        def _add(question, reason, intent, priority=50, focus_metric='', compare_mode=None,
                 target_types=None, target_levels=None):
        ...
        for name,pct,dw in (hl_non_l1 if hl_non_l1 else hl_sorted)[:3]:
            ...
            if '下降' in str(dw):
                _add(f'分析{pfx}{period}【{name}】下降的主要拖累项',
                     f'【{name}】为本轮主要下降项', 'variance_risk_drilldown',
                     priority=100, focus_metric=name, target_levels=['2','3'])
            else:
                _add(f'分析{pfx}{period}【{name}】变动的主要拉动项',
                     f'【{name}】为本轮主要异动项', 'variance_drilldown',
                     priority=95, focus_metric=name, target_levels=['2','3'])
        ...
        for name,pct,dw,_ in (risk_non_l1 if risk_non_l1 else risk_sorted)[:2]:
            ...
            _add(f'查看{pfx}{period}【{name}】是否存在连续下降或异常波动',
                 ...
                 priority=90, focus_metric=name, compare_mode='trend', target_levels=['1','2'])
        ...
        for name,_,_ in sorted(contribution_items, key=lambda x:x[2], reverse=True)[:2]:
            ...
            _add(f'下钻查看{pfx}{period}【{name}】明细构成及贡献占比',
                 f'【{name}】存在可解释的结构/占比信息', 'contribution_drilldown',
                 priority=85, focus_metric=name, target_levels=['2','3'])
            ...
            for name,_,_ in drill[:2]:
                if name:
                    _add(f'进一步查看{pfx}{period}【{name}】明细构成',
                         ...
                         priority=70, focus_metric=name, target_levels=['2','3'])
        ...
        if is_point or is_date_shift:
            ...
            _add(f'下钻分析{pfx}{period}{type_cn}指标的主要构成变化',
                 '当前为趋势分析，建议进一步看结构归因', 'trend_to_detail',
                 priority=60, target_levels=['2','3'])
