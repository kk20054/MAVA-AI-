...
def build_suggestion_candidates(raw, dd, asset_prefix):
    ...
    def add_suggestion(topic, observation, judgment, action, followup):
        sugs.append({
            'topic': topic,
    ...
    if dep_total and dep_total != 0 and low_int is not None:
        ...
        if casa < 40:
            add_suggestion(
                '优化负债结构',
                f'低息存款占客户存款{casa:.2f}%',
                '低成本负债占比偏低，负债成本仍有优化空间',
                '围绕结算沉淀、代发代扣和重点客群维护提升低成本资金留存，优先跟进有稳定结算流水但活期沉淀不足的客户',
                '后续重点跟踪低息存款占比、活期/储蓄存款留存和定期迁徙情况'
            )
        if low_lye_g is not None and low_lye_g < 0:
            add_suggestion(
                '稳定低息资金来源',
                f'低息存款较上年末下降{pct_str(low_lye_g)}',
                '核心结算资金留存承压，需防止低成本资金继续流失',
                '排查重点客户资金流出、到期转存及结算活跃度变化，优先稳住核心结算户和工资代发、收单、现金管理等场景客户',
                '后续跟踪重点客户余额变动、结算频次和低息存款日均修复情况'
            )
        if fix_lye_g is not None and fix_lye_g > 0.2:
            add_suggestion(
                '控制存款成本',
                f'定期存款较上年末增长{pct_str(fix_lye_g)}',
                '定期化趋势较明显，规模增长可能伴随付息成本上行',
                '同步关注付息成本和到期分布，对高成本定期存款做好续作分层和价格管理，避免规模增长对净利息收入形成挤压',
                '后续跟踪定期存款占比、到期峰值和净利息收入变化'
            )
    ...
    if normal and normal != 0:
        ...
        if loan_types:
            ...
            if loan_types[0][1] > 50:
                add_suggestion(
                    '分散贷款结构集中度',
                    f'{loan_types[0][0]}占正常贷款{loan_types[0][1]:.2f}%',
                    '贷款品种集中度偏高，需关注单一品种波动对资产端规模和收益的影响',
                    '结合客户行业、抵质押和期限结构进一步下钻，区分优质增量、自然还款和潜在风险暴露，适度拓展其他稳健贷款品种',
                    '后续跟踪前三大贷款品种占比、重点客户集中度和正常贷款余额变化'
                )
    ...
    if loan_lye_g is not None and loan_lye_g < 0:
        add_suggestion(
            '修复贷款规模动能',
            f'贷款余额较上年末下降{pct_str(loan_lye_g)}',
            '资产端投放动能不足，可能削弱贷款利息收入的后续支撑',
            '梳理存量客户提款、还款和授信使用情况，优先推动优质客户授信转化，并对还款较快或额度闲置客户开展补充需求挖掘',
            '后续跟踪授信使用率、新增提款、还款集中客户和贷款时点余额修复情况'
        )
    ...
    if npl and npl != 0 and npl_lye_g is not None and npl_lye_g > 0:
        add_suggestion(
            '强化资产质量管控',
            f'不良贷款较上年末上升{pct_str(npl_lye_g)}',
            '资产质量出现压力信号，需前移风险识别和处置节奏',
            '将风险客户列入重点跟踪清单，结合逾期、担保和还款来源推进分类管理，对可修复客户明确还款安排，对处置类客户推动压降',
            '后续跟踪不良贷款余额、拨备覆盖、逾期迁徙和重点客户处置进度'
        )
    ...
    if rev_total and rev_total != 0 and nii is not None:
        ...
        if nii_pct > 85:
            add_suggestion(
                '提升收入多元化',
                f'净利息收入占营业净收入{nii_pct:.2f}%',
                '收入对息差业务依赖较高，非息收入对营业净收入的补充作用仍需增强',
                '围绕结算、代发、贸易金融及财富类场景提升非息贡献，优先在存贷款重点客户中叠加中间业务产品',
                '后续跟踪中间业务净收入占比、收费及佣金收入和重点客户产品渗透率'
            )
    if fee is not None:
        ...
        if fee_yoy_g is not None and fee_yoy_g < -0.1:
            add_suggestion(
                '修复中间业务收入',
                f'中间业务净收入累计同比下降{pct_str(fee_yoy_g)}',
                '非息收入表现承压，可能影响营业净收入结构和利润稳定性',
                '拆分收费及佣金来源，定位拖累产品并跟进客户渗透率，对结算、贸易金融、托管及财富类产品形成分客群推进清单',
                '后续跟踪收费及佣金收入、产品覆盖率和重点客户中收贡献'
            )
    ...
    if rev_mom_g is not None and rev_mom_g > 0.1:
        add_suggestion(
            '巩固收入增长动能',
            f'营业净收入当月环比增长{pct_str(rev_mom_g)}',
            '收入端短期动能较好，应识别主要贡献来源并提升可复制性',
            '复盘主要贡献客户和产品，将有效打法沉淀为后续拓展清单，同时关注增长是否来自一次性收益或可持续业务',
            '后续跟踪营业净收入当月环比、主要客户贡献和产品收入延续性'
        )
    if rev_mom_g is not None and rev_mom_g < -0.1:
        add_suggestion(
            '排查收入环比下行',
            f'营业净收入当月环比下降{pct_str(rev_mom_g)}',
            '收入端短期波动较大，需识别是息差、规模还是非息收入拖累',
            '优先核查净利息收入、中收和财资业务的环比拖累项，分客户、分产品定位缺口来源，并形成修复措施',
            '后续跟踪当月营业净收入、三大收入板块环比和重点客户贡献变化'
        )
    ...
    if not sugs:
        add_suggestion(
            '保持经营跟踪闭环',
            '整体经营表现未见明显异常',
            '当前更适合以稳态经营和结构优化为主，避免只追求单项规模增长',
            '保持规模、收益、结构和风险四类指标的月度跟踪，并结合重点客户开展机会挖掘',
            '后续跟踪营业净收入、贷款、客户存款、低息存款占比和资产质量指标'
        )
